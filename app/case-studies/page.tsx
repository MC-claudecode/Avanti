...
function StatBar() {
  ...
  return (
    <section ref={barRef} style={{ background: "#fff", padding: "48px 28px", borderBottom: "1px solid rgba(0,0,0,0.06)" }}>
      <div className="cs-stats-bar" style={{ maxWidth: 860, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 24, textAlign: "center" }}>
        {stats.map((s, i) => (
          <div key={s.n}>
            <div className="cs-stat-num" style={{ fontSize: 48, fontWeight: 600, color: T.ink, letterSpacing: "-0.03em" }}>
              <span ref={el => { counterRefs.current[i] = el; }}>0</span>
              {i === 1 ? "%" : i === 2 ? "%" : ""}
            </div>
            <div className="cs-stat-label" style={{ fontSize: 14, color: T.ink48, marginTop: 6, lineHeight: 1.4 }}>{s.l}</div>
          </div>
        ))}
      </div>
    </section>
...
function CaseStudySection({ c, i }: { c: typeof caseStudiesData[0]; i: number }) {
  ...
  return (
    <section ref={sectionRef} className="cs-section" style={{ background: bg, padding: "92px 28px" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <div ref={headerRef} className="responsive-stack" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 60, alignItems: "end", marginBottom: 48 }}>
          <div>
            ...
            <h2 className="cs-section-headline" style={{ fontSize: "clamp(28px,3vw,42px)", fontWeight: 700, color: head, letterSpacing: "-0.03em", lineHeight: 1.08, marginBottom: 14 }}>{c.headline}</h2>
          ...
          <div className="cs-headline-card" style={{ background: cardBg, borderRadius: 18, border: cardBorder, padding: "32px 28px", textAlign: "left", boxShadow: c.dark ? "0 12px 40px rgba(10,132,255,0.16)" : "0 8px 32px rgba(15,20,25,0.06)", position: "relative", overflow: "hidden" }}>
            ...
            <div className="brand-gradient-text cs-headline-result" style={{ fontSize: "clamp(56px,6vw,84px)", fontWeight: 700, letterSpacing: "-0.045em", lineHeight: 1, position: "relative" }}>{c.result}</div>
        ...
        <div style={{ marginTop: 48 }}>
          ...
          <div className="cs-strategy-grid" style={{ display: "grid", gridTemplateColumns: c.phases.length === 4 ? "repeat(2, 1fr)" : "repeat(auto-fit, minmax(260px, 1fr))", gap: 16 }}>
            {c.phases.map((p, pi) => (
        ...
        {/* Metrics */}
        <div className="cs-metrics-grid" style={{ marginTop: 40, display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 0, borderTop: `1px solid ${ruleCol}`, borderBottom: `1px solid ${ruleCol}` }}>
          {c.metrics.map((m, mi) => (
            <div key={mi} className="cs-metric-cell" style={{ padding: "26px 18px", borderRight: mi < c.metrics.length - 1 ? `1px solid ${ruleCol}` : "none" }}>
        ...
        {/* Outcome + Services */}
        <div className="cs-outcome-grid" style={{ marginTop: 36, display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 40, alignItems: "start" }}>
          <div>
  ...
  return (
    <>
      ...
      <div style={{ paddingTop: 52 }}>
        {/* Hero */}
        <section className="cs-hero-section" style={{ position: "relative", background: "linear-gradient(140deg, #06121A 0%, #0E1A22 50%, #0A2A2A 100%)", padding: "160px 28px 120px", overflow: "hidden" }}>
          ...
          <div style={{ maxWidth: 1200, margin: "0 auto", position: "relative" }}>
            <div className="responsive-stack" style={{ display: "grid", gridTemplateColumns: "minmax(0, 1.4fr) minmax(0, 1fr)", gap: 60, alignItems: "center" }}>
              <div>
                ...
                <h1 className="cs-hero-headline" style={{ fontSize: "clamp(56px, 9vw, 132px)", fontWeight: 700, color: "#fff", letterSpacing: "-0.045em", lineHeight: 0.92, marginBottom: 28 }}>
                  The work,<br/>
                  <span className="brand-gradient-text">in numbers.</span>
                </h1>
                <p className="cs-hero-subhead" style={{ fontSize: "clamp(18px, 1.6vw, 22px)", color: T.paper65, lineHeight: 1.45, letterSpacing: "-0.2px", maxWidth: 540 }}>
